...
export type BusinessSettings = { business_name: string; tagline: string; location: string; opening_hours: string; contact_email: string; business_type: string; whatsapp_number: string; owner_phone: string }
export type PolicySettings = { delivery_policy: string; returns_policy: string; payment_methods: string[]; cancellation_policy: string; max_discount_ceiling: number }
export type PermissionSettings = { can_create_orders: boolean; can_create_quotes: boolean; can_confirm_delivery_date: boolean; negotiation_enabled: boolean; max_discount_agent_can_approve: number; markup: number; floor: number; escalate_on: string[] }
...
export const api = {
  ...
  getBusiness: () => request<BusinessSettings>('/api/settings/business'), updateBusiness: (settings: BusinessSettings) => request('/api/settings/business', { method: 'PATCH', body: JSON.stringify(settings) }),
  getPolicies: () => request<PolicySettings>('/api/settings/policies'), updatePolicies: (settings: PolicySettings) => request('/api/settings/policies', { method: 'PATCH', body: JSON.stringify(settings) }),
  getPermissions: () => request<PermissionSettings>('/api/settings/permissions'), updatePermissions: (settings: PermissionSettings) => request('/api/settings/permissions', { method: 'PATCH', body: JSON.stringify(settings) }),
